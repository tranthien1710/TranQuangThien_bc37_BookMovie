...
const reduce = (state = initialState, action) => {
    switch (action.type) {
        case 'CHANGE_CHAIR': {
            const data = [...state.selectedChair];
            const foundIdex = data.findIndex((item) => item.soGhe === action.payload.soGhe)
            if (foundIdex !== -1) {
                data.splice(foundIdex, 1)
            }
            else {
                data.push(action.payload)
            }
            state.selectedChair = data;
            return { ...state }
        }
        case "CHANGE_PAY": {
            const data = [...state.listChair]
            data.forEach(item => {
                item.danhSachGhe.forEach((ele) => {
                    state.selectedChair.forEach((item) => {
                        if (ele.soGhe === item.soGhe) {
                            ele.daDat = true;
                        }
                    })
                })
        ...
        case 'DELETE': {
            const data = [...state.selectedChair];
            const foundindex = data.findIndex(item => item.soGhe === action.payload.soGhe)
